# Retrofuturism & Speculative-Fiction Genres

*30 styles in this family (7 flagship). Generated from `scripts/style_catalog.json` — the single source of truth.*

How different eras imagined the future — cyberpunk, synthwave, Y2K, Frutiger Aero, solarpunk, and the whole '-punk' family (steam/diesel/atom/cassette). Each encodes a specific decade's optimism or dread. They're highly evocative and very palette/texture-driven.

> **How to apply this family.** **Nail the era's tech-materials and palette first** — neon-on-black for cyberpunk, glossy aqua for Frutiger Aero, sunset-grid for synthwave. Glow, grain, and scanlines are signature but must respect `prefers-reduced-motion` and never bury body-text contrast.

## Contents
- [Cyberpunk](#cyberpunk) ⭐
- [Frutiger Aero](#frutiger-aero) ⭐
- [Lunarpunk](#lunarpunk) ⭐
- [Solarpunk](#solarpunk) ⭐
- [Synthwave / Outrun](#synthwave--outrun) ⭐
- [Vaporwave](#vaporwave) ⭐
- [Y2K Futurism](#y2k-futurism) ⭐
- [Atompunk](#atompunk)
- [Biopunk](#biopunk)
- [Cassette Futurism](#cassette-futurism)
- [Chromecore](#chromecore)
- [Clockpunk](#clockpunk)
- [Cyberprep](#cyberprep)
- [Decopunk](#decopunk)
- [Dieselpunk](#dieselpunk)
- [Dreampunk](#dreampunk)
- [Formicapunk](#formicapunk)
- [Googie](#googie)
- [Laserwave / Darksynth](#laserwave--darksynth)
- [McBling](#mcbling)
- [Nanopunk](#nanopunk)
- [Raygun Gothic](#raygun-gothic)
- [Seapunk](#seapunk)
- [Silicon Dreams](#silicon-dreams)
- [Silkpunk](#silkpunk)
- [Space Age Pop](#space-age-pop)
- [Steampunk](#steampunk)
- [Stonepunk](#stonepunk)
- [Tidalpunk](#tidalpunk)
- [Vectordelia](#vectordelia)

---

### Cyberpunk  ⭐ *flagship — full spec in [00-flagship-implementation-specs/cyberpunk.md](00-flagship-implementation-specs/cyberpunk.md)*
*Aliases:* cyber, high tech low life, neon noir  
*Slug:* `cyberpunk` · *Era:* 1982–present

**Origin.** Literary roots (Gibson 'Neuromancer' 1984); visual codex from Blade Runner (1982); revived by Cyberpunk 2077.

**Defining traits.**
- Neon glow (cyan/magenta) on near-black
- Dense HUD/glitch overlays, mono type, scanlines
- Rain-slick megacity noir, holograms
- High-contrast, dystopian, tech-saturated

**Reference example.** Blade Runner; Cyberpunk 2077 UI; Ghost in the Shell.

**Don't confuse with:** synthwave, vaporwave, glitch-art.

---

### Frutiger Aero  ⭐ *flagship — full spec in [00-flagship-implementation-specs/frutiger-aero.md](00-flagship-implementation-specs/frutiger-aero.md)*
*Aliases:* Web 2.0 gloss, Aero, cleancore  
*Slug:* `frutiger-aero` · *Era:* 2004–2013

**Origin.** Retroactively named (after Frutiger typeface + Windows Aero); nature-meets-tech optimism.

**Defining traits.**
- Glossy skeuomorphic bubbles and glass
- Nature imagery: water, bubbles, grass, blue skies
- Aqua gradients, lens flares, clean tech
- Humanist Frutiger/Myriad type, auto-glow

**Reference example.** Windows Vista/7; Wii/iOS-era wallpapers; Windows XP 'Bliss'.

**Don't confuse with:** glassmorphism, y2k-futurism, solarpunk.

---

### Lunarpunk  ⭐ *flagship — full spec in [00-flagship-implementation-specs/lunarpunk.md](00-flagship-implementation-specs/lunarpunk.md)*
*Aliases:* night solarpunk, moonpunk, dark solarpunk  
*Slug:* `lunarpunk` · *Era:* 2020s–present

**Origin.** Emerged online ~2020s as the nocturnal, mystical counterpart to solarpunk within eco-futurist communities.

**Defining traits.**
- Moonlit bioluminescence over deep indigo/black
- Dark botanical and celestial motifs (moths, moonflowers, star charts)
- Appropriate-tech-at-night: solar batteries, glow-lanterns, quiet machinery
- Soft glowing edges instead of hard neon; mystical over corporate

**Reference example.** Lunarpunk art communities on Tumblr/DeviantArt; 'nocturnal eco-futurism' zine covers.

**Don't confuse with:** solarpunk, cyberpunk, dreampunk.

---

### Solarpunk  ⭐ *flagship — full spec in [00-flagship-implementation-specs/solarpunk.md](00-flagship-implementation-specs/solarpunk.md)*
*Aliases:* eco-futurism  
*Slug:* `solarpunk` · *Era:* 2008–present

**Origin.** Emerged online ~2008 as optimistic counter to cyberpunk.

**Defining traits.**
- Lush greenery fused with clean technology
- Warm sunlit palette: greens, gold, sky blue, terracotta
- Art-Nouveau-influenced organic linework
- Hopeful, sustainable, community-centered

**Reference example.** Solarpunk art collections; 'Chobani' Solarpunk ad (2021).

**Don't confuse with:** frutiger-aero, art-nouveau, cottagecore.

---

### Synthwave / Outrun  ⭐ *flagship — full spec in [00-flagship-implementation-specs/synthwave.md](00-flagship-implementation-specs/synthwave.md)*
*Aliases:* outrun, retrowave, retro-futurism 80s  
*Slug:* `synthwave` · *Era:* 2009–present

**Origin.** Music-driven revival of 1980s sci-fi/action visuals.

**Defining traits.**
- Neon grid horizon and giant gradient sun
- Magenta/cyan on deep blue-purple night
- Chrome 80s type, palm trees, sports cars
- Retro-futuristic, cinematic, VHS glow

**Reference example.** Kavinsky 'Nightcall'; Hotline Miami; Far Cry Blood Dragon.

**Don't confuse with:** vaporwave, cyberpunk.

---

### Vaporwave  ⭐ *flagship — full spec in [00-flagship-implementation-specs/vaporwave.md](00-flagship-implementation-specs/vaporwave.md)*
*Aliases:* vapor, aesthetic (A E S T H E T I C)  
*Slug:* `vaporwave` · *Era:* 2010–present

**Origin.** Internet music/visual microgenre (~2010–2012).

**Defining traits.**
- Roman busts, 80s/90s consumer nostalgia, glitch
- Pink/purple/cyan sunset gradients
- Japanese katakana, marble, palm trees, grids
- Ironic, dreamy, mall-nostalgic

**Reference example.** Macintosh Plus 'Floral Shoppe'; vaporwave album art.

**Don't confuse with:** synthwave, seapunk, mallsoft.

---

### Y2K Futurism  ⭐ *flagship — full spec in [00-flagship-implementation-specs/y2k-futurism.md](00-flagship-implementation-specs/y2k-futurism.md)*
*Aliases:* Y2K, millennium bug aesthetic, cyber-y2k  
*Slug:* `y2k-futurism` · *Era:* 1995–2004

**Origin.** Turn-of-millennium optimistic tech aesthetic.

**Defining traits.**
- Glossy translucent 'blobjects' and chrome
- Metallic silver, icy blues, hot pink accents
- Bubble/inflatable UI, lens flares, starbursts
- Techno-optimism, iMac G3 / bubble tech

**Reference example.** iMac G3; The Matrix marketing; early Windows XP era ads.

**Don't confuse with:** frutiger-aero, vaporwave, chromecore.

---

### Atompunk
*Aliases:* atomic age, raygun gothic-adjacent  
*Slug:* `atompunk` · *Era:* 1998–present

**Origin.** Retrofuturism of the atomic age (1945–1965).

**Defining traits.**
- Googie curves, boomerangs, starbursts
- Space-race optimism, atoms and rockets
- Teal, orange, chrome, formica
- Populuxe consumer futurism

**Reference example.** Fallout series UI; The Jetsons; mid-century ads.

**Don't confuse with:** googie, raygun-gothic, space-age.

---

### Biopunk
*Aliases:* organic sci-fi, biotech  
*Slug:* `biopunk` · *Era:* 1990s–present

**Origin.** Offshoot of cyberpunk focused on biotechnology.

**Defining traits.**
- Wet organic textures, flesh and fungus
- Bio-luminescence, veins, membranes
- Genetic/lab motifs
- Body-horror-adjacent futurism

**Reference example.** The Thing; Scorn; Annihilation.

**Don't confuse with:** cyberpunk, organic-design.

---

### Cassette Futurism
*Aliases:* cassette punk, analog sci-fi  
*Slug:* `cassette-futurism` · *Era:* 2010s–present (evokes 1970s–80s)

**Origin.** Fan term for retro-analog sci-fi tech aesthetics.

**Defining traits.**
- Chunky beige/grey plastic hardware
- CRT monitors, blinking LEDs, physical switches
- Monospace amber/green terminal type
- Lo-fi analog 'future as imagined in 1979'

**Reference example.** Alien (Nostromo); Severance; Fallout Pip-Boy.

**Don't confuse with:** ascii-terminal, crt, dieselpunk.

---

### Chromecore
*Aliases:* liquid chrome, metal type  
*Slug:* `chromecore` · *Era:* 2020s (evokes Y2K)

**Origin.** Revived Y2K metallic aesthetic.

**Defining traits.**
- Liquid/reflective chrome surfaces and type
- Silver, mercury, iridescent reflections
- Blobby metallic 3D forms
- Futuristic, sleek, cold

**Reference example.** 2020s album covers; fashion (Balenciaga, Rui).

**Don't confuse with:** y2k-futurism, holographic, chrome-metal.

---

### Clockpunk
*Aliases:* Renaissance punk, da Vinci punk  
*Slug:* `clockpunk` · *Era:* 2000s–present

**Origin.** Punk genre set in Renaissance-era clockwork tech.

**Defining traits.**
- Clockwork gears, springs, brass automata
- Da Vinci-style sketches and mechanisms
- Renaissance materials: wood, brass
- Analog pre-steam mechanical fantasy

**Reference example.** Assassin's Creed (Leonardo's machines).

**Don't confuse with:** steampunk.

---

### Cyberprep
*Aliases:* optimistic cyberpunk  
*Slug:* `cyberprep` · *Era:* 1990s–present

**Origin.** Optimistic counterpart to dystopian cyberpunk (Frutiger-adjacent).

**Defining traits.**
- Clean, bright, safe high-tech future
- White/aqua/green, glossy surfaces
- Seamless friendly tech integration
- Utopian consumer sci-fi

**Reference example.** Mirror's Edge; early 2000s tech ads.

**Don't confuse with:** frutiger-aero, cyberpunk.

---

### Decopunk
*Aliases:* Deco-punk, raygun deco  
*Slug:* `decopunk` · *Era:* 2010s–present

**Origin.** Sleeker, brighter offshoot of dieselpunk (1920s–40s).

**Defining traits.**
- Polished Art Deco chrome and glamour
- Brighter palette than gritty dieselpunk
- Streamlined luxury machine age
- Gold, cream, chrome, geometric

**Reference example.** BioShock (Rapture); The Great Gatsby (2013).

**Don't confuse with:** dieselpunk, art-deco, streamline-moderne.

---

### Dieselpunk
*Aliases:* interwar retrofuturism  
*Slug:* `dieselpunk` · *Era:* 2001–present

**Origin.** Term coined ~2001; aesthetic of 1920s–1940s diesel era.

**Defining traits.**
- Interwar/WWII industrial machine age
- Art Deco + military-industrial forms
- Muted metals, propaganda-poster palette
- Streamlined engines, zeppelins, riveted steel

**Reference example.** BioShock; Sky Captain and the World of Tomorrow.

**Don't confuse with:** atompunk, art-deco, steampunk.

---

### Dreampunk
*Aliases:* oneiric aesthetic  
*Slug:* `dreampunk` · *Era:* 2010s–present

**Origin.** Surreal, dream-logic offshoot of vaporwave/ambient.

**Defining traits.**
- Hazy, ethereal, foggy gradients
- Soft glow, indistinct forms
- Liminal, otherworldly calm
- Ambient, hypnagogic

**Reference example.** Dreampunk music visuals; 2814 album art.

**Don't confuse with:** vaporwave, dreamcore.

---

### Formicapunk
*Aliases:* 1980s corporate futurism  
*Slug:* `formicapunk` · *Era:* 2010s (evokes 1980s)

**Origin.** Neon-lit 1980s corporate/consumer futurism.

**Defining traits.**
- Formica, neon, geometric 80s corporate style
- Miami-Vice pastels + chrome
- Grid patterns, glass towers
- Optimistic 80s consumer future

**Reference example.** Miami Vice; 80s corporate design.

**Don't confuse with:** synthwave, memphis-design.

---

### Googie
*Aliases:* populuxe, doo-wop architecture  
*Slug:* `googie` · *Era:* 1945–1969

**Origin.** Southern California car-culture architecture.

**Defining traits.**
- Upswept roofs, boomerangs, starbursts, atoms
- Bold neon signage, cantilevers
- Space-age optimism
- Amoeba/kidney-bean shapes

**Reference example.** LAX Theme Building; retro motel/diner signs.

**Don't confuse with:** atompunk, streamline-moderne, space-age.

---

### Laserwave / Darksynth
*Aliases:* darksynth, dreamwave  
*Slug:* `laserwave` · *Era:* 2013–present

**Origin.** Darker, harder subgenre of synthwave.

**Defining traits.**
- Aggressive neon on black, horror tones
- Red/purple/cyan, chrome skulls
- Cinematic, ominous
- VHS grain and glow

**Reference example.** Perturbator; Carpenter Brut visuals.

**Don't confuse with:** synthwave, cyberpunk.

---

### McBling
*Aliases:* bling era, 2000s glam  
*Slug:* `mcbling` · *Era:* 2003–2008

**Origin.** Mid-2000s pop-culture glamour aesthetic.

**Defining traits.**
- Rhinestones, glitter, hot pink, gold bling
- Flip-phone/tabloid celebrity culture
- Juicy/velour, Y2K glam
- Ostentatious, playful excess

**Reference example.** Paris Hilton era; early Myspace glam.

**Don't confuse with:** y2k-futurism, barbiecore.

---

### Nanopunk
*Aliases:* nanotech futurism  
*Slug:* `nanopunk` · *Era:* 2000s–present

**Origin.** Cyberpunk offshoot centered on nanotechnology.

**Defining traits.**
- Swarming particle/grey-goo motifs
- Clinical near-future tech
- Molecular grid overlays
- Smart-material morphing surfaces

**Reference example.** Prey; various near-future sci-fi UIs.

**Don't confuse with:** cyberpunk, biopunk.

---

### Raygun Gothic
*Aliases:* retro sci-fi, googie sci-fi  
*Slug:* `raygun-gothic` · *Era:* 1930s–1960s (revived)

**Origin.** Term popularized by William Gibson; pulp sci-fi + Googie.

**Defining traits.**
- Sleek rockets, ray guns, chrome fins
- Streamlined optimistic space age
- Bold geometric sans + starbursts
- 1950s 'World of Tomorrow' futurism

**Reference example.** The Jetsons; Forbidden Planet; Tomorrowland.

**Don't confuse with:** atompunk, googie, space-age.

---

### Seapunk
*Aliases:* aquatic vaporwave  
*Slug:* `seapunk` · *Era:* 2011–2013

**Origin.** Tumblr-born internet microgenre.

**Defining traits.**
- Aquatic teal/cyan, dolphins, waves
- 90s CGI/Web 1.0 nostalgia
- Yin-yang, glitch, holographic
- Watery, kitschy, ironic

**Reference example.** Seapunk Tumblr; Rihanna 2012 SNL visuals.

**Don't confuse with:** vaporwave, vectordelia, y2k-futurism.

---

### Silicon Dreams
*Aliases:* Frutiger Metro  
*Slug:* `silicon-dreams` · *Era:* 2005–2012

**Origin.** Frutiger-family aesthetic; darker tech-corporate variant.

**Defining traits.**
- Dark glossy tech backgrounds
- Blue glow, circuitry, HUD hints
- Corporate futurism
- Glass + neon-blue accents

**Reference example.** Mid-2000s corporate tech keynotes.

**Don't confuse with:** frutiger-aero, cyberprep.

---

### Silkpunk
*Aliases:* East-Asian retrofuturism  
*Slug:* `silkpunk` · *Era:* 2015–present

**Origin.** Coined by Ken Liu; East-Asian-inspired bio/mechanical tech.

**Defining traits.**
- Bamboo, silk, paper, lacquer materials
- Organic bio-mechanics (airships, kites)
- Classical Chinese/Asian motifs
- Elegant, flowing, nature-integrated tech

**Reference example.** Ken Liu 'The Grace of Kings'.

**Don't confuse with:** steampunk, biopunk.

---

### Space Age Pop
*Aliases:* space age, 1960s futurism  
*Slug:* `space-age` · *Era:* 1957–1972

**Origin.** Space-race-era design optimism.

**Defining traits.**
- Sleek white + primary pops, molded plastic
- Spherical/pod forms, chrome
- Op-art and geometric patterns
- Astronaut-era optimism

**Reference example.** 2001: A Space Odyssey interiors; Panton furniture.

**Don't confuse with:** atompunk, mid-century-modern.

---

### Steampunk
*Aliases:* Victorian sci-fi, gaslamp  
*Slug:* `steampunk` · *Era:* 1980s–present

**Origin.** Coined 1987 (K.W. Jeter); Victorian-era speculative tech.

**Defining traits.**
- Brass, copper, wood, leather materials
- Exposed gears, cogs, rivets, pipes, gauges
- Ornate Victorian serif type and filigree
- Analog steam-powered anachronistic tech

**Reference example.** The Difference Engine; Wild Wild West; BioShock Infinite-adjacent.

**Don't confuse with:** dieselpunk, victorian.

---

### Stonepunk
*Aliases:* prehistoric punk  
*Slug:* `stonepunk` · *Era:* 2000s–present

**Origin.** Punk genre using stone-age technology.

**Defining traits.**
- Stone, bone, wood 'technology'
- Earthy, rough, primitive materials
- Cave-painting motifs
- Anachronistic prehistoric tech

**Reference example.** The Flintstones; The Croods.

**Don't confuse with:** steampunk.

---

### Tidalpunk
*Aliases:* oceanpunk  
*Slug:* `tidalpunk` · *Era:* 2019–present

**Origin.** Ocean-focused optimistic offshoot of solarpunk.

**Defining traits.**
- Ocean tech: wave/tidal energy, kelp, coral
- Blues, teals, salvaged materials
- Sustainable seafaring future
- Hopeful, aquatic, resourceful

**Reference example.** Tidalpunk art community.

**Don't confuse with:** solarpunk, seapunk.

---

### Vectordelia
*Aliases:* Web 2.0 vectors, swirl graphics  
*Slug:* `vectordelia` · *Era:* 2004–2010

**Origin.** Mid-2000s vector-art trend (part of Frutiger family).

**Defining traits.**
- Swooping vector swirls and 'wirebirds'
- Glossy gradients, floral flourishes
- Bright saturated color
- Adobe Illustrator flourish era

**Reference example.** Mid-2000s tech ads; Windows Vista promo art.

**Don't confuse with:** frutiger-aero, y2k-futurism.

---
